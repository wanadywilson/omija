...
void _updateTaxPercentage() {
  ...
  taxPercentageController.text = percentage.toStringAsFixed(2);
...
void _updateServiceChargePercentage() {
  ...
  final percentage = subtotal > 0 ? (amount / subtotal) * 100 : 0;
  serviceChargePercentageController.text = percentage.toStringAsFixed(2);
  ...
  Widget _buildTaxField(String label, TextEditingController percent, TextEditingController amount) {
    return Row(
      children: [
        Expanded(child: TextField(controller: percent, decoration: InputDecoration(labelText: "$label %"))),
        SizedBox(width: 10),
        Expanded(child: TextField(controller: amount, decoration: InputDecoration(labelText: "$label Amount"))),
      ],
    );
  }
